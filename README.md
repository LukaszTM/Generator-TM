...
5. **Generowanie**:
   ...
   - **przypadki testowe** (ID, moduł, typ, priorytet, warunki wstępne, kroki, dane testowe, oczekiwany rezultat) — reguły rozpoznają m.in. logowanie, rejestrację, formularze, wyszukiwanie, filtry, listy, import/eksport plików, płatności, API, uprawnienia, raporty, ustawienia i daty; generowane są testy pozytywne, negatywne, brzegowe i bezpieczeństwa (do wyboru). **Tryb nauki**: każdy przypadek ma wyjaśnienie „💡 Dlaczego ten test?” z nazwą techniki testowej (analiza wartości brzegowych, klasy równoważności…), a wbudowany **słowniczek testera** (20 pojęć) tłumaczy terminologię — program uczy testowania w trakcie pracy.
6. **Wykonywanie testów (test runner)** — przechodzisz przypadek po przypadku: widzisz kroki i oczekiwany rezultat, klikasz **Zaliczony / Niezaliczony / Zablokowany**, dopisujesz notatkę, a program sam przechodzi do kolejnego niewykonanego przypadku i pokazuje postęp. Przycisk **„Zgłoś błąd…”** oznacza przypadek jako niezaliczony i wypełnia formularz zgłoszenia jego danymi (kroki, oczekiwany rezultat, powiązanie z przypadkiem). Na koniec zapisujesz **raport z wykonania testów** (HTML/MD/CSV) z podsumowaniem, rozbiciem na moduły i listą niezaliczonych.
7. **Eksport** do `.md` (Markdown), `.csv` (średniki — przyjazne polskiemu Excelowi, importowalne do Jiry/TestRaila) oraz `.html` (gotowy do druku).
8. **Testy stron WWW** — podajesz adres URL, a program wykonuje automatyczne kontrole: dostępność i kod odpowiedzi, czas reakcji, HTTPS (w tym sprawdzenie, czy strona HTTP ma wersję szyfrowaną), tytuł/meta description/viewport/atrybut lang/H1, teksty alternatywne obrazów, favicon, mieszaną treść (HTTP na HTTPS), nagłówki bezpieczeństwa odpowiedzi oraz **działanie odnośników** znalezionych na stronie (do 20). Wyniki (OK/UWAGA/BŁĄD/INFO) można zapisać jako raport HTML/MD/CSV albo jednym kliknięciem przenieść jako zgłoszenia do raportu błędów.
9. **Raport błędów** — formularz zgłoszenia (tytuł, moduł, powiązany przypadek testowy, waga, środowisko, kroki reprodukcji, rezultat aktualny/oczekiwany) z możliwością **dołączenia zrzutów ekranu** z pliku (`.png`, `.jpg`, `.webp`, `.bmp`; można wybrać kilka naraz). Eksport raportu do:
...
W nagłówku okna jest przełącznik **Motyw** z ośmioma wariantami zbudowanymi z pakietów assetów TestPilot Studio:
...
- **Futurystyczny — jasny / ciemny** — pakiet nazwany Futuristic (`assets/futuristic_light/`, `assets/futuristic_dark/`): pastylkowe przyciski ze stanami i gradientowym primary, świecące obramowania, comboboxy, checkboxy, paski przewijania, tabele i dedykowane ikony 16/24/32 px,
...
Uwaga do pakietu Modern Light: elementy powierzchni (karty, pola, toolbar) w plikach PNG tej paczki mają jednolity niebieski kolor niezgodny z jej `style_tokens.json` i `preview.png` — dlatego jasny motyw jest rysowany bezpośrednio z tokenów kolorów, a z paczki używane są ikony.
