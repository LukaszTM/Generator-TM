...
7. **Testy stron WWW** — podajesz adres URL, a program wykonuje automatyczne kontrole: dostępność i kod odpowiedzi, czas reakcji, HTTPS (w tym sprawdzenie, czy strona HTTP ma wersję szyfrowaną), tytuł/meta description/viewport/atrybut lang/H1, teksty alternatywne obrazów, favicon, mieszaną treść (HTTP na HTTPS), nagłówki bezpieczeństwa odpowiedzi oraz **działanie odnośników** znalezionych na stronie (do 20). Wyniki (OK/UWAGA/BŁĄD/INFO) można zapisać jako raport HTML/MD/CSV albo jednym kliknięciem przenieść jako zgłoszenia do raportu błędów.
8. **Raport błędów** — formularz zgłoszenia (tytuł, moduł, powiązany przypadek testowy, waga, środowisko, kroki reprodukcji, rezultat aktualny/oczekiwany) z możliwością **dołączenia zrzutów ekranu** z pliku (`.png`, `.jpg`, `.webp`, `.bmp`; można wybrać kilka naraz). Eksport raportu do:
